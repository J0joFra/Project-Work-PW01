...
def fetch_attachment_link(ingredient_url):
    response = requests.get(ingredient_url)
    if response.status_code == 200:
        soup = BeautifulSoup(response.content, 'html.parser')
        attachment = soup.find(id='ContentContainer_ContentBottom_ingredientReferences')
        if attachment:
            link_tag = attachment.find('a', href=True)
            if link_tag:
...
def create_workbook():
    ...
    headers = ['pcpc_ingredientid', 'pcpc_ingredientname', 'link', 'attachment_link']  # Intestazioni colonne
    ...
    try:
        ...
        for url in urls:
            ...
            for element in desired_elements:
                attachment_link = fetch_attachment_link(element[2])
                element_with_attachment = element + (attachment_link,)
                row_idx = write_data_to_excel(ws, [element_with_attachment], start_row=row_idx)
